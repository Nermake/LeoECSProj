...
namespace Leopotam.Ecs {
...
#endif
    public sealed class EcsSystems : IEcsInitSystem, IEcsDestroySystem, IEcsRunSystem, IEcsFixedRunSystem, IEcsLateRunSystem {
        ...
        readonly Dictionary<Type, object> _injections = new (32);
        bool _injected;
#if DEBUG
        bool _initialized;
        bool _destroyed;
        readonly List<IEcsSystemsDebugListener> _debugListeners = new (4);
        ...
        public EcsSystems Add (IEcsSystem system, string namedRunSystem = null) {
            ...
            if (system is IEcsFixedRunSystem) {
                if (namedRunSystem == null && system is EcsSystems ecsSystems) {
                    namedRunSystem = ecsSystems.Name;
                }
                if (namedRunSystem != null) {
#if DEBUG
                    if (_namedFixedRunSystems.ContainsKey (namedRunSystem.GetHashCode ())) {
                        throw new Exception ($"Cant add named system - \"{namedRunSystem}\" name already exists.");
                    }
#endif
            ...
            if (system is IEcsLateRunSystem) {
                ...
                if (namedRunSystem != null) {
#if DEBUG
                    if (_namedLateRunSystems.ContainsKey (namedRunSystem.GetHashCode ())) {
                        throw new Exception ($"Cant add named system - \"{namedRunSystem}\" name already exists.");
                    }
#endif
        ...
        public int GetNamedRunSystem (string name) {
            return _namedRunSystems.GetValueOrDefault(name.GetHashCode (), -1);
        }
        
        public int GetNamedFixedRunSystem (string name) {
            return _namedFixedRunSystems.GetValueOrDefault(name.GetHashCode (), -1);
        }
        
        public int GetNamedLateRunSystem (string name) {
            return _namedLateRunSystems.GetValueOrDefault(name.GetHashCode (), -1);
        }
        ...
        /// </summary>
        private EcsSystems ProcessInjects () {
#if DEBUG
            if (_initialized) { throw new Exception ("Cant inject after initialization."); }
            if (_destroyed) { throw new Exception ("Cant touch after destroy."); }
#endif
    ...
    public sealed class EcsSystemsFixedRunItem {
        public bool Active;
        public IEcsFixedRunSystem System;
    }
    
    public sealed class EcsSystemsLateRunItem {
        public bool Active;
        public IEcsLateRunSystem System;
    }
}